	...
	public class ProjectileScript : MonoBehaviour
	{
		...
		private void OnCollisionEnter(Collision collision)
		{
            ...
            if (collision.gameObject.tag == "Enemy" || collision.gameObject.tag == "EnemyBody")
            {
                ...
                //Toggle the isHit bool on the target object
                collision.gameObject.transform.GetComponentInParent<EnemyNavigation>().CheckHit(power, power * Vector3.forward, transform.position);
            }
			...
			foreach (Collider hit in colliders)
			{
                ...
                if (hit.GetComponent<Collider>().tag == "Enemy" || hit.GetComponent<Collider>().tag == "EnemyBody")
                {

                    hit.gameObject.transform.GetComponentInParent<EnemyNavigation>().CheckHit(power, power * Vector3.forward, hit.transform.position);
                }
